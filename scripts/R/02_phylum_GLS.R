...
#merge taxonomy in .tsv
summary <- read.csv("summary_clean.tsv", sep="\t")
taxonomy <- read.csv("mapfile.tsv", sep="\t")
taxonomy$accession <- sub(".*(GCA_[0-9]+|GCF_[0-9]+).*", "\\1", taxonomy$accession) #clean accession 
names(summary)[names(summary) == 'species'] <- 'accession' #rename column

merged <- merge(summary,taxonomy, by="accession", all.x=TRUE)

write.table(merged, "merged_summary.tsv", sep="\t",
...
#create dataframe
data_min <- merged[, c("accession", "phylum", "ogt", "genome_size",
...
#identify phyla with enough power
min_n <- 20
phyla_counts <- table(data2$phylum)
...
for (phy in good_phyla) {
  
  cat("Running:", phy, "\n")
  
  sub_data <- data2 %>% filter(phylum == phy)
...
# SAVE SUMMARY TABLE 
write.table(results_table, "per_phylum_analysis.tsv", sep="\t",
